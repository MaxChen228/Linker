...
from core.exceptions import (
    UnifiedError, ErrorCategory, ErrorSeverity, LinkerError,
    DatabaseError, FileIOError, NetworkError, BusinessLogicError, 
    ConcurrencyError, SystemError
)
...
class ErrorHandler:
    ...
    def _convert_to_unified_error(self, error: Exception) -> UnifiedError:
        ...
        category, error_code, severity = error_mappings.get(
            error_type, 
            (ErrorCategory.UNKNOWN, "UNKNOWN_ERROR", ErrorSeverity.MEDIUM)
        )
        ...
        return UnifiedError(
            ...
            severity=severity,
            details={
                "original_type": error_type.__name__,
    ...
    def _should_fallback(self, error: UnifiedError) -> bool:
        ...
        return (
            error.category in fallback_categories and 
            error.severity in fallback_severities and
