...
class ErrorCategory(Enum):
    ...
    def from_string(cls, value: str) -> "ErrorCategory":
        ...
        return next(
            (member for name, member in cls.__members__.items() if name.lower() == value.lower()),
            cls.OTHER,
        )
    ...
    def get_review_multiplier(self) -> float:
        """獲取用於計算下次複習時間的間隔乘數。"""
        return {self.SYSTEMATIC: 0.8, self.ISOLATED: 1.0, self.OTHER: 1.0, self.ENHANCEMENT: 1.5}[
            self
        ]
...
class ErrorTypeSystem:
    ...
    @classmethod
    def classify(
        cls, key_point: str, explanation: str, severity: str = "major"
        ...
        if severity == "minor" or any(
            kw in combined_text for kw in ["更自然", "更道地", "更好", "不夠自然"]
        ):
