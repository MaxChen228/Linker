...
class VersionManager:
    ...
    def needs_migration(self, file_path: Path, file_type: FileType) -> Tuple[bool, Optional[DataVersion]]:
        """檢查是否需要遷移"""
    ...
    def migrate_file(self, file_path: Path, file_type: FileType) -> bool:
        ...
        while current != target_version:
            ...
            if current is None:
                ...
                current_str = f"v{current.value.replace('.', '_')}"
        ...
        logger.info(f"Successfully migrated {file_path} to {current.value if current else 'unknown'}")
        return True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "check":
            ...
            # 備份所有檔案
            for file in ['grammar_patterns.json', 'patterns_enriched_complete.json', 'knowledge.json']:
