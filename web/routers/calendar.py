...
def calculate_study_intensity(daily_record: dict) -> str:
    ...
    total_activities = (
        len(daily_record.get("completed_reviews", [])) +
        daily_record.get("new_practices", 0)
    )
...
async def get_day_details(date_str: str):
        ...
        if hasattr(kp, 'next_review') and kp.next_review:
            ...
            if review_date == target_date:
                due_reviews.append({
                    "id": kp.id,
                    "key_point": kp.key_point,
                    "original_phrase": kp.original_phrase if hasattr(kp, 'original_phrase') else "",
    ...
    return {
        ...
        "study_sessions": [s for s in calendar_data.get("study_sessions", []) if s.get("date") == date_str]
    }
