...
async def update_tags(point_id: int, tags: list[str]):
    ...
    return JSONResponse({"success": True, "message": "標籤已更新", "tags": tags})
...
async def update_notes(point_id: int, notes: str):
    ...
    return JSONResponse({"success": True, "message": "筆記已更新", "notes": notes})
...
@router.post("/batch")
async def batch_operation(request: BatchRequest, background_tasks: BackgroundTasks):
    """批量操作端點"""
    knowledge = get_knowledge_manager()
