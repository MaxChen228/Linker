...
async def test_daily_practice_complete_cycle():
    ...
    assert len(common_candidates) > 0, (
        f"沒有共同的複習候選: JSON={json_candidate_keys}, DB={db_candidate_keys}"
    )
    ...
    json_mistake_result = json_manager.save_mistake(
        new_practice["chinese_sentence"], new_practice["user_answer"], new_practice["feedback"]
    )
...
async def test_mixed_practice_session():
    ...
    expected_correct_count = len(
        [p for p in mixed_practices if p["feedback"]["is_generally_correct"]]
    )
    expected_mistake_count = len(
        [p for p in mixed_practices if not p["feedback"]["is_generally_correct"]]
    )
...
async def test_review_learning_cycle():
    ...
    json_manager.save_mistake(practice["chinese"], practice["user_answer"], practice["feedback"])
...
async def test_performance_consistency():
    ...
    assert performance_ratio < 10, (
        f"數據庫模式過慢: JSON={json_time:.3f}s, DB={db_time:.3f}s, 比率={performance_ratio:.2f}"
    )
