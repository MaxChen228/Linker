...
class TestConcurrentOperations:
    ...
    async def test_concurrent_read_write(
        self, create_test_knowledge_point, mock_json_manager, mock_db_manager, edge_case_test_config
    ):
        """測試併發讀寫操作的數據一致性"""
        ...
        async def reader_task():
            ...
            for i in range(50):
                ...
                current_total = sum(
                    p.mistake_count + p.correct_count for p in mock_json_manager.knowledge_points
                )
        ...
        async def writer_task():
            ...
            for i in range(20):
                ...
                new_point = create_test_knowledge_point(
                    id=1000 + i, key_point=f"動態添加知識點 {i + 1}"
                )
                ...
                stats_tracker["total_practices"] += (
                    new_point.mistake_count + new_point.correct_count
                )
        ...
        for i, (json_stats, db_stats) in enumerate(reader_results):
            ...
            assert 20 <= json_count <= 40, f"知識點數量超出預期範圍: {json_count}"
    ...
    async def test_cache_consistency_under_load(
        ...
        # 模擬快取管理器
        cache_stats = {"hit_count": 0, "miss_count": 0, "total_requests": 0}
        ...
        async def load_generator():
            ...
            for i in range(100):
                ...
                if random.choice([True, False]):
                    ...
                    task = asyncio.create_task(asyncio.coroutine(lambda: expected_stats)())
                else:
                    # 模擬快取未命中
    ...
    async def test_concurrent_statistics_calculation(
        self, create_test_knowledge_point, mock_json_manager, mock_db_manager
    ):
        """測試併發統計計算的正確性"""
        # 創建多樣化的測試數據
        test_points = []
    ...
    async def test_race_condition_prevention(
        ...
        async def modify_data_task(task_id: int):
            ...
            for i in range(5):
                # 模擬數據修改操作
                new_point = create_test_knowledge_point(
                    id=task_id * 100 + i, key_point=f"任務{task_id}添加的點{i}"
                )
    ...
    @pytest.mark.asyncio
    async def test_deadlock_prevention(self, mock_json_manager, mock_db_manager):
        """測試死鎖防範機制"""
        # 模擬可能導致死鎖的操作序列
        lock_a = asyncio.Lock()
        lock_b = asyncio.Lock()
