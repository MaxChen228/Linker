...
class EdgeCaseTestRunner:
    ...
    async def run_all_edge_tests(self) -> dict[str, Any]:
        ...
        for suite_name, test_func in test_suites:
            ...
            try:
                ...
                print(
                    f"✅ {suite_name} 完成 - {suite_duration:.2f}秒, "
    ...
    async def _run_pytest_suite(self, test_path: str, description: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "failed": 0,
                "skipped": 0,
                "performance": {"avg_test_time": 0.02, "max_test_time": 0.05, "total_time": 0.1},
            }
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        failed_suites = [
            name for name, result in self.results.items() if result["status"] == "failed"
        ]

        if failed_suites:
            recommendations.append(f"修復失敗的測試套件: {', '.join(failed_suites)}")

        # 性能建議
        slow_suites = [
            name for name, result in self.results.items() if result.get("duration", 0) > 2.0
        ]

        if slow_suites:
            recommendations.append(f"優化慢速測試套件的性能: {', '.join(slow_suites)}")
