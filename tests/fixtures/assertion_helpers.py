        ...
        if isinstance(json_val, (int, float)) and isinstance(db_val, (int, float)):
            if json_val > 0:
                ...
                assert diff_rate <= tolerance, (
                    f"{field} 不一致: JSON={json_val}, DB={db_val}, 差異={diff_rate:.4f}"
                )
            else:
...
def assert_initial_state_consistent(json_stats: Dict, db_stats: Dict, expected: Dict) -> None:
    ...
    for key, expected_value in expected.items():
        if key in ["recommendations"]:
            ...
            assert json_stats.get(key) == expected_value, (
                f"JSON {key} 不符預期: {json_stats.get(key)} != {expected_value}"
            )
            assert db_stats.get(key) == expected_value, (
                f"DB {key} 不符預期: {db_stats.get(key)} != {expected_value}"
            )
...
def assert_stats_match_expected(stats: Dict, expected: Dict) -> None:
    ...
    for key, expected_value in expected.items():
        if key == "category_distribution":
            ...
            assert stats.get(key) == expected_value, (
                f"{key} 不符預期: {stats.get(key)} != {expected_value}"
            )
...
def assert_knowledge_point_consistent(json_point: KnowledgePoint, db_point: KnowledgePoint) -> None:
    ...
    assert json_point.key_point == db_point.key_point, (
        f"知識點內容不一致: {json_point.key_point} != {db_point.key_point}"
    )
    assert json_point.category == db_point.category, (
        f"分類不一致: {json_point.category} != {db_point.category}"
    )
    assert abs(json_point.mastery_level - db_point.mastery_level) < 0.01, (
        f"掌握度不一致: {json_point.mastery_level} != {db_point.mastery_level}"
    )
    assert json_point.is_deleted == db_point.is_deleted, (
        f"刪除狀態不一致: {json_point.is_deleted} != {db_point.is_deleted}"
    )
...
def assert_operation_results_consistent(
    json_result: Any, db_result: Any, operation_name: str = ""
) -> None:
    """斷言操作結果一致性"""
    assert json_result == db_result, (
        f"{operation_name} 操作結果不一致: JSON={json_result}, DB={db_result}"
    )


def assert_response_time_acceptable(
    json_time: float, db_time: float, tolerance_percent: float = 0.2
) -> None:
    """斷言響應時間差異在可接受範圍內"""
    if json_time > 0:
        diff_rate = abs(json_time - db_time) / max(json_time, db_time)
        assert diff_rate <= tolerance_percent, (
            f"響應時間差異過大: JSON={json_time:.3f}s, DB={db_time:.3f}s, 差異={diff_rate:.3f}"
        )
...
def assert_error_consistency(json_error: str, db_error: str) -> None:
    """斷言錯誤訊息一致性"""
    # 允許細微的格式差異，但核心訊息應該一致
    assert json_error.lower().strip() == db_error.lower().strip(), (
        f"錯誤訊息不一致: JSON='{json_error}', DB='{db_error}'"
    )
