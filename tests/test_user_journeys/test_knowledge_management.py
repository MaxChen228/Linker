...
async def test_knowledge_point_lifecycle_management():
    ...
    if hasattr(db_manager, 'update_knowledge_point_async'):
        db_edit_result = await db_manager.update_knowledge_point_async(target_point_id, **edit_updates)
    ...
    if hasattr(db_manager, 'delete_knowledge_point_async'):
        db_delete_result = await db_manager.delete_knowledge_point_async(target_point_id, delete_reason)
    else:
...
async def test_batch_knowledge_operations():
    ...
    for point_id in target_ids:
        if hasattr(db_manager, 'update_knowledge_point_async'):
            ...
            result = db_manager.edit_knowledge_point(point_id, batch_updates) if hasattr(db_manager, 'edit_knowledge_point') else True
    ...
    for point_id in target_ids:
        assert point_id not in remaining_json_ids, f"JSON 刪除的知識點 {point_id} 不應該在活躍列表中"
...
async def test_knowledge_point_search_consistency():
    ...
    for term in search_terms:
        ...
        if json_results and db_results:
            ...
            # 計算一致性比例
            consistency_ratio = len(common_keys) / len(total_unique_keys) if total_unique_keys else 1.0
