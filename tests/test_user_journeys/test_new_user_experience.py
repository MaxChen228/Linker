...
async def test_new_user_multiple_mistakes():
    ...
    for i, mistake in enumerate(mistakes):
        ...
        expected_knowledge_points = len([m for m in mistakes[:i+1] if not m['feedback']['is_generally_correct']])
        expected_correct_count = len([m for m in mistakes[:i+1] if m['feedback']['is_generally_correct']])
...
async def test_new_user_first_success():
    ...
    # 處理正確答案
    json_result = json_manager.save_mistake(
        success_practice['chinese'],
        success_practice['user_answer'],
        success_practice['feedback']
    )
