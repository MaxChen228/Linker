...
async def test_comprehensive_search_experience():
    ...
    # === 階段 2: 分類過濾 ===
    for category in [ErrorCategory.SYSTEMATIC, ErrorCategory.ISOLATED,
                    ErrorCategory.ENHANCEMENT, ErrorCategory.OTHER]:
    ...
    for min_mastery, max_mastery, level_name in mastery_ranges:
        json_filtered = [p for p in json_all_points
                        if min_mastery <= p.mastery_level < max_mastery]
        db_filtered = [p for p in db_all_points
                      if min_mastery <= p.mastery_level < max_mastery]
...
async def test_comprehensive_statistics_journey():
    ...
    for metric in key_metrics:
        ...
        if isinstance(json_val, (int, float)) and isinstance(db_val, (int, float)):
            if json_val > 0:
                ...
                assert diff_ratio <= 0.2, f"{metric} 差異過大: JSON={json_val}, DB={db_val}, 差異={diff_ratio:.4f}"
            else:
                assert db_val == 0 or db_val == json_val, f"{metric} 不一致: JSON={json_val}, DB={db_val}"
    ...
    for category in common_categories:
        ...
        if json_count > 0:
            ...
            assert diff_ratio <= 0.3, f"分類 {category} 數量差異過大: JSON={json_count}, DB={db_count}"
    ...
    if json_candidate_count > 0 and db_candidate_count > 0:
        diff_ratio = abs(json_candidate_count - db_candidate_count) / max(json_candidate_count, db_candidate_count)
        assert diff_ratio <= 0.4, f"複習候選數量差異過大: JSON={json_candidate_count}, DB={db_candidate_count}"
...
async def test_search_result_ranking_consistency():
    ...
    if len(json_results) > 1 and len(db_results) > 1:
        ...
        if len(common_ids) >= 2:
            ...
            # 允許部分順序差異
            order_similarity = sum(1 for i, id in enumerate(json_top_common) if i < len(db_top_common) and id == db_top_common[i])
...
async def test_statistical_trend_analysis():
    ...
    if json_stats_updated['total_practices'] > 0 and db_stats_updated.get('total_practices', 0) > 0:
        json_success_rate = json_stats_updated['correct_count'] / json_stats_updated['total_practices']
        db_success_rate = db_stats_updated['correct_count'] / db_stats_updated['total_practices'] if db_stats_updated['total_practices'] > 0 else 0
        ...
        if json_success_rate > 0 or db_success_rate > 0:
            ...
            assert success_rate_diff <= 0.3, f"成功率差異過大: JSON={json_success_rate:.3f}, DB={db_success_rate:.3f}"
    ...
    if json_stats_updated.get('avg_mastery', 0) > 0 and db_stats_updated.get('avg_mastery', 0) > 0:
        ...
        assert mastery_diff <= 0.2, f"平均掌握度差異過大: JSON={json_avg:.3f}, DB={db_avg:.3f}, 差異={mastery_diff:.3f}"
