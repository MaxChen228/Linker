...
def assert_stats_consistent(stats1: Dict[str, Any], stats2: Dict[str, Any], tolerance: float = 0.000001):
    """驗證兩個統計結果的一致性"""
    # 獲取所有鍵
...
def assert_user_session_consistent(session1: Dict[str, Any], session2: Dict[str, Any]):
    ...
    for operation_name, (result1, result2) in session1.items():
        ...
        if operation_name == 'statistics':
            ...
            assert len(result1) == len(baseline1), \
                f"操作 {operation_name} JSON 結果長度不一致"
            assert len(result2) == len(baseline2), \
                f"操作 {operation_name} DB 結果長度不一致"


def recommendations_consistent(rec1: Dict[str, Any], rec2: Dict[str, Any]) -> bool:
    """檢查推薦結果是否一致"""
    return (
        rec1.get('next_review_count') == rec2.get('next_review_count') and
        len(rec1.get('recommendations', [])) == len(rec2.get('recommendations', []))
    )
