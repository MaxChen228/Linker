...
class ArchitectureDesignAnalyzer:
    ...
    def _analyze_api_interface_design(self) -> dict:
        ...
        # 計算設計一致性
        consistent_methods = sum(1 for analysis in interface_analysis.values() 
                               if analysis['design_consistent'])
        design_consistency = consistent_methods / len(core_business_methods)
        
        print(f"   核心業務方法設計一致性: {design_consistency:.1%} ({consistent_methods}/{len(core_business_methods)})")
        
        # 分析設計差異
        missing_methods = [method for method, analysis in interface_analysis.items() 
    ...
    async def _analyze_business_logic_design(self) -> dict:
        ...
        # 計算業務邏輯設計一致性
        consistent_logic = sum(1 for analysis in logic_analysis.values() 
                              if analysis.get('design_consistent', False))
        logic_consistency = consistent_logic / len(logic_analysis) if logic_analysis else 0
        
        print(f"   業務邏輯設計一致性: {logic_consistency:.1%} ({consistent_logic}/{len(logic_analysis)})")
    ...
    def _analyze_method_signatures(self) -> dict:
        ...
        consistent_signatures = sum(1 for analysis in signature_analysis.values() 
                                   if analysis.get('design_consistent', False))
        signature_consistency = (consistent_signatures / len(signature_analysis) 
                               if signature_analysis else 0)
    ...
    def _analyze_architectural_patterns(self) -> dict:
        ...
        consistent_patterns = sum(1 for pattern in patterns_analysis.values() 
    ...
    def _generate_design_assessment(self, results: dict) -> dict:
        ...
        # 提取各項設計分數
        scores = {
            'api_interface_design': results.get('api_interface_design', {}).get('design_consistency', 0),
            'data_model_design': results.get('data_model_design', {}).get('design_score', 0),
            'business_logic_design': results.get('business_logic_design', {}).get('consistency_score', 0),
            'method_signature_design': results.get('method_signature_analysis', {}).get('consistency_score', 0),
            'error_handling_design': results.get('error_handling_design', {}).get('consistency_score', 0),
            'architectural_patterns': results.get('architectural_patterns', {}).get('consistency_score', 0)
        }
