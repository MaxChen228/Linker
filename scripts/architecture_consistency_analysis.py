...
class RealArchitectureAnalyzer:
    ...
    async def _analyze_api_interfaces(self) -> Dict[str, Any]:
        ...
        # 檢查核心方法存在性
        core_methods = [
            'get_statistics', 'get_active_points', 'get_review_candidates',
            'get_learning_recommendations', 'edit_knowledge_point',
        ...
        for method in core_methods:
            ...
            # 檢查Database管理器的異步版本
            async_method = method + '_async' if method != 'get_active_points' else 'get_knowledge_points_async'
        ...
        print(f"   核心方法兼容性: {compatibility_score:.1%} ({available_methods}/{len(core_methods)})")
    ...
    async def _analyze_statistics_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   統計一致性: {consistency_score:.1%} ({matching_metrics}/{len(key_metrics)} 指標匹配)")
    ...
    async def _analyze_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 計算平均性能比率
            avg_ratio = sum(metric['ratio'] for metric in performance_results.values()) / len(performance_results)
            
            print(f"   統計計算: JSON={json_stats_time:.4f}s, DB={db_stats_time:.4f}s (比率: {performance_results['statistics_calculation']['ratio']:.2f})")
            print(f"   數據檢索: JSON={json_retrieval_time:.4f}s, DB={db_retrieval_time:.4f}s (比率: {performance_results['data_retrieval']['ratio']:.2f})")
            print(f"   平均性能比率: {avg_ratio:.2f}")
            
            performance_results['average_ratio'] = avg_ratio
            performance_results['performance_score'] = min(1.0, 2.0 / avg_ratio) if avg_ratio > 0 else 1.0
    ...
    def _analyze_method_availability(self) -> Dict[str, Any]:
        ...
        # 獲取所有方法
        json_methods = [method for method in dir(self.json_manager) 
                       if not method.startswith('_') and callable(getattr(self.json_manager, method))]
        db_methods = [method for method in dir(self.db_manager) 
    ...
    def _generate_overall_assessment(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 提取各項分數
        scores = {
            'api_compatibility': results.get('api_interface_analysis', {}).get('compatibility_score', 0.0),
            'data_model_consistency': 1.0 if results.get('data_model_analysis', {}).get('model_consistency') else 0.0,
            'statistics_consistency': results.get('statistics_consistency', {}).get('consistency_score', 0.0),
            'performance_consistency': results.get('performance_comparison', {}).get('performance_score', 0.0),
            'method_availability': results.get('method_availability', {}).get('method_overlap', 0.0),
            'error_handling': results.get('error_handling_comparison', {}).get('overall_consistency', 0.0)
        }
