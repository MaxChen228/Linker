    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                "http://localhost:8000/api/grade-answer", json=test_data, timeout=30.0
            )
...
async def test_generate_question():
    ...
    test_data = {"mode": "new", "length": 1, "level": 2}

    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                "http://localhost:8000/api/generate-question", json=test_data, timeout=30.0
            )
